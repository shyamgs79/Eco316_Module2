%Main program - this version useful to detect punctuated equilibria

%This version assumes that hardships are constant for an agent over time

 %Also this version assumes that jail terms are imposed randomly across 
 %time, but equally across people arrested at the same time
...
for t=1:numruns
     ...
     waitbar((t-1)/numruns,ha)
     
     %Jail terms imposed randomly across time,  but equally across people
     J_term=round(J_max*rand);
    ...
    for k=randperm(numagents+numcops)
        ...
        if k<=numagents && S_agent(k)==0
            ...
            if J_agent(k)<=0
                ...
                %Only jailed agents cannot move, those just out of jail do
                continue
            end
        ...
        if k>numagents
            ...
            J_agent(arrestedagentnum)=J_term;
            %countactive(t)=countactive(t)-1; 
