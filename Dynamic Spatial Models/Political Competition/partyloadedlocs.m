...
numt=500; %number of time periods for which to observe dynamics

partytypes=[1,1]; %this vector contains the number and type of each party
...
xyrange=1+2*max([max(x),abs(min(x)), max(y), abs(min(y))]); %find the max range - height or width, also dsire symetry about 0.
...
for t=2:numt
    ...
    for p = randperm(numparties)
        ...
        elseif partytypes(p)==3
            ...
            %implicitly, do nothing if x ccordinate is the same
